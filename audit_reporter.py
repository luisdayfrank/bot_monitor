...
class AuditReporter:
    """
    Generador de reportes de auditoría diarios — V5.5 Granular.

    FASE 5.5 CAMBIOS:
    ─────────────────
    • Manejo de eventos CONTINUO (cada vela 15m)
    • Manejo de eventos NEAR_MISS (casi-disparos)
    • Manejo de eventos METRICAS_DIA (agregados)
    • Análisis de evolución del score durante el día
    • Detección de "horas muertas" vs "horas activas"
    """
    ...
    async def _generar_y_enviar_reporte(self):
        ...
        try:
            ...
            # Enviar archivos JSON + limpieza post-envío
            archivos_borrados = 0
            ...
            for filepath, filename in archivos_generados:
                try:
                    ...
                    if enviado:
                        # ✅ Envío confirmado → borrar archivo
                        try:
                            os.remove(filepath)
                            archivos_borrados += 1
                            print(f"  ✅ Enviado + 🗑️ Borrado: {filename}")
                        except Exception as e_borrar:
                            print(f"  ✅ Enviado pero ⚠️ NO borrado: {filename} | Error: {e_borrar}")
                            archivos_conservados += 1
                    else:
                        # ❌ Envío falló → conservar archivo para reintento
                        archivos_conservados += 1
                        print(f"  ❌ NO enviado (conservado): {filename}")
                        
                except Exception as e_envio:
                    archivos_conservados += 1
                    print(f"  ❌ Error enviando {filename}: {e_envio} | Archivo conservado en {filepath}")
            ...
            if archivos_conservados > 0:
                await self.notifier.enviar_telegram(
                    f"⚠️ <b>Auditoría {fecha_ayer}</b>\n"
                    f"{archivos_conservados} archivo(s) JSON no pudieron enviarse y permanecen en el VPS.\n"
                    f"Ubicación: <code>{db_dir}</code>\n"
                    f"Revisa conectividad con Telegram o envía manualmente."
                )
    ...
    async def _generar_json_moneda(self, symbol: str, fecha: str, eventos: List[dict]) -> dict:
        """Genera el JSON completo para una moneda con auditoría granular."""
        ...
        eventos_continuo = [e for e in eventos if e['tipo'] == 'CONTINUO']  # NUEVO
        eventos_near_miss = [e for e in eventos if e['tipo'] == 'NEAR_MISS']  # NUEVO
        eventos_metricas = [e for e in eventos if e['tipo'] == 'METRICAS_DIA']  # NUEVO

        # Procesar eventos principales (sin CONTINUO)
        eventos_procesados = []
        for ev in eventos:
            if ev['tipo'] == 'CONTINUO':
                continue  # Los continuos van en sección separada
        ...
        # FASE 5.5: Procesar logs continuos para análisis de evolución
        ...
        recomendaciones = self._generar_recomendaciones(eventos, seguimiento_post, eventos_near_miss)
        ...
        resultado = {
            ...
            "evolucion_score": evolucion_score,  # NUEVO
            "eventos": eventos_procesados,
            "near_misses": near_misses_procesados,  # NUEVO
    ...
    def _analizar_evolucion_score(self, eventos_continuo: List[dict]) -> dict:
        """
        Analiza la evolución del score durante el día a partir de los logs continuos.
        """
        if not eventos_continuo:
            return {
                "mensaje": "No hay logs continuos para este día",
                "score_max": None,
                "score_min": None,
                "score_promedio": None,
        ...
        if not scores:
            return {
                "mensaje": "Logs continuos sin scores válidos",
                "score_max": None,
                "score_min": None,
                "score_promedio": None,
    ...
    def _detectar_horas_activas(self, scores_por_hora: dict) -> list:
        """Detecta horas con scores más altos (más cerca de disparar)."""
    ...
    def _detectar_horas_menos_activas(self, scores_por_hora: dict) -> list:
        """Detecta horas con scores más bajos (mercado plano)."""
        if not scores_por_hora:
            return []
        promedios = [(hora, sum(vals)/len(vals)) for hora, vals in scores_por_hora.items()]
    ...
    def _generar_recomendaciones(self, eventos, seguimiento_post, eventos_near_miss):
        ...
        # FASE 5.5: Recomendación basada en near-misses
        if near_miss_count >= 5:
            recomendaciones.append({
                "parametro": "umbrales_generales",
                "actual": "65-75",
                "sugerido": "60-70 (más permisivo)",
                "motivo": f"{near_miss_count} near-misses detectados. El bot estuvo cerca de disparar múltiples veces. "
                          "Considerar bajar umbrales para capturar más oportunidades."
            })
    ...
    def _generar_resumen_texto(self, fecha: str, eventos_por_moneda: Dict) -> str:
        ...
        lineas = [
            ...
            f"📈 Logs continuos: {total_continuos} (cada vela 15m)",
        ]

        if total_near_miss > 0:
            lineas.append(f"🎯 Near-misses: {total_near_miss} (casi disparos)")
        ...
        for symbol, eventos in sorted(eventos_por_moneda.items()):
            ...
            extras = []
            if continuos > 0:
                extras.append(f"📈 {continuos} logs")
            if near > 0:
                extras.append(f"🎯 {near} near-miss")
        ...
        lineas.append("💡 Los logs continuos muestran la evolución del score cada vela 15m.")
        lineas.append("🎯 Los near-misses indican oportunidades que el bot casi capturó.")
