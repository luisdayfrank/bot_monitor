...
class AuditReporter:
    """
    Generador de reportes de auditoría diarios — V5.5.

    Se ejecuta una vez al día, genera un JSON por moneda con todos
    los eventos y seguimiento, y lo envía como archivo adjunto por Telegram.

    FASE 5.5 CAMBIOS:
    ─────────────────
    • Versión actualizada a 5.5
    • Manejo de eventos tipo SNAPSHOT (auditoría granular)
    • Umbral "BLOQUEADO_ADX_EXTREMO" en vez de 999 en reportes
    • Recomendaciones para nuevos parámetros de Fase 5.5
    """
    ...
    async def _generar_y_enviar_reporte(self):
        ...
        try:
            ...
            # Enviar archivos JSON como adjuntos
    ...
    async def _generar_json_moneda(self, symbol: str, fecha: str, eventos: List[dict]) -> dict:
        ...
        # FASE 5.5: Separar eventos por tipo incluyendo SNAPSHOT
        ...
        eventos_snapshot = [e for e in eventos if e['tipo'] == 'SNAPSHOT']  # NUEVO

        # Construir lista de eventos procesados (sin snapshots en la lista principal)
        eventos_procesados = []
        snapshots_procesados = []  # NUEVO: lista separada para snapshots
        ...
        for ev in eventos:
            ...
            if ev.get('contexto_json'):
                try:
                    ...
                    # FASE 5.5: Si el contexto tiene umbral_bloqueado, mostrarlo legible
            ...
            # FASE 5.5: Separar snapshots del flujo principal
            if ev['tipo'] == 'SNAPSHOT':
                snapshots_procesados.append(evento_proc)
            else:
        ...
        # FASE 5.5: Construir JSON con snapshots separados
        resultado = {
            "meta": {
                "symbol": symbol,
                "fecha": fecha,
                "modo": "auditoria_externa",
                "version_bot": "5.5",  # FASE 5.5: Actualizado
                "timezone": CONFIG.timezone,
                "hora_generacion": now_local().isoformat(),
                "total_eventos": len(eventos),
                "total_snapshots": len(eventos_snapshot)  # NUEVO
            },
            "resumen_dia": {
                ...
                "snapshots": len(eventos_snapshot),  # NUEVO
        ...
        # FASE 5.5: Incluir snapshots solo si existen
        if snapshots_procesados:
            resultado["snapshots_periodicos"] = snapshots_procesados
    ...
    def _generar_preguntas_evaluacion(self, eventos_fire, eventos_rechazados, seguimiento_post):
        ...
        for ev in eventos_fire:
            ...
            # FASE 5.5: Extraer umbral del contexto si existe
            umbral_info = ""
            try:
        ...
        for ev in eventos_rechazados:
            ...
            # FASE 5.5: Detectar umbral bloqueado en rechazos
            ...
            nota_extra = ""
            if umbral_bloqueado:
                nota_extra = " | 🔒 Umbral bloqueado por ADX"
            if mfm_rechazo:
                nota_extra += " | ⚠️ Rechazado por MFM contradictorio"
    ...
    def _generar_recomendaciones(self, eventos, seguimiento_post):
        ...
        umbrales_bloqueados = 0  # NUEVO: contar bloqueos por ADX
        ...
        # Recomendación para ADX extremo
        ...
        # FASE 5.5: Si hay muchos bloqueos por ADX, sugerir revisar umbrales
        if umbrales_bloqueados >= 5:
            recomendaciones.append({
                "parametro": "umbral_adx",
                "actual": "45 (extremo) / 20 (bajo)",
                "sugerido": "Revisar si el mercado está en rango lateral prolongado",
                "motivo": f"{umbrales_bloqueados} eventos con umbral bloqueado por ADX. "
                          "El bot está demasiado conservador o el mercado no tiene tendencia."
            })
        ...
        # FASE 5.5: Recomendación para commitment score si hay muchos rechazos
        # por score insuficiente pero cerca del umbral
        rechazos_score = rechazos_frecuentes.get('Score insuficiente', 0)
        if rechazos_score >= 3:
            recomendaciones.append({
                "parametro": "commitment_umbral",
                "actual": "50 (Fase 5.5)",
                "sugerido": "45 (más permisivo)",
                "motivo": f"{rechazos_score} rechazos por score insuficiente cerca del umbral. "
                          "Considerar bajar el umbral de commitment."
            })
    ...
    def _generar_resumen_texto(self, fecha: str, eventos_por_moneda: Dict) -> str:
        ...
        total_snapshots = sum(len([e for e in evs if e['tipo'] == 'SNAPSHOT']) for evs in eventos_por_moneda.values())
        ...
        if total_snapshots > 0:
            lineas.append(f"📸 Snapshots: {total_snapshots} (auditoría granular)")

        if mfm_rechazos > 0:
            lineas.append(f"📉 Rechazos MFM: {mfm_rechazos}")

        if umbrales_bloqueados > 0:
            lineas.append(f"🔒 Umbrales bloqueados (ADX): {umbrales_bloqueados}")
        ...
        for symbol, eventos in sorted(eventos_por_moneda.items()):
            ...
            snapshots = len([e for e in eventos if e['tipo'] == 'SNAPSHOT'])
            snap_str = f" | 📸 {snapshots}" if snapshots > 0 else ""
            lineas.append(f"• {symbol}: {fires} FIRE, {rechazos} RECHAZOS{snap_str}")

        lineas.append("")
        lineas.append("📎 Revisa los archivos JSON adjuntos para el análisis completo.")
        lineas.append("💡 Los snapshots muestran la evolución del score cada 4h.")
